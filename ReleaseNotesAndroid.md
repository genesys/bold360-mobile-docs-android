...
### Bot related
- **UserId management** - To ensure that the sessions from a device belong to the same user in reporting we generate a unique userId. This id is now generated by the BE, and available for embedding Apps, via `AccountInfoProvider.update` implementation. 
```diff
- Breaking Changes
```
...
- **Multi answer design** - multiple answers on bot response (kind - inlineChoice) are displayed as persistent, and are not disappears on option selection.

### Fixes
- Voice to voice - The response to a recorded user query that was edited before send, will not be read up.
- Memory management - Improve SDK allocated resources release.
